...
2026-01-30: Added Streamlit chunk retrieval app tailored to this repo with RRF/vector/PPR/edge options plus Cypher agent tab. Cypher agent now returns per-step queries/results (normalized Neo4j types) so the UI can show full traces. Eval F1: not run.
2026-01-30: Fixed Streamlit import to load cypher_agent module dynamically and handle streaming availability; live steps render via generator when available. Eval F1: not run.
